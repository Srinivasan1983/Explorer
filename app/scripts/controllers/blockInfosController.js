...
angular.module('ethExplorer')
    .controller('blockInfosCtrl', function ($rootScope, $scope, $location, $routeParams,$q,$timeout) {
        ...
        $rootScope.init=function()
        {
            ...
            if($scope.blockId!==undefined) {
                ...
                getBlockInfos()
                    ...
                    $rootScope.blockNumber = result.number;
                    ...
                    if ($rootScope.blockNumber !== undefined){
                            $scope.conf = number - $rootScope.blockNumber + " Confirmations";
                            if (number === $rootScope.blockNumber){
                                $scope.conf = 'Unconfirmed';
                                $scope.lastBlock = true;
                            }
                            if ($rootScope.blockNumber === 0) {
                                $scope.firstBlock = true;
                            }
                        }

                        if ($rootScope.blockNumber !== undefined){
                            var info = web3.eth.getBlock($rootScope.blockNumber);
                            if (info !== undefined){
